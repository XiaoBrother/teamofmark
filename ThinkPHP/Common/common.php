...
function import($class, $baseUrl = '', $ext='.class.php') {
    ...
    if (empty($baseUrl)) {
        ...
        if ('@' == $class_strut[0] || APP_NAME == $class_strut[0]) {
            ...
            $class=str_replace("_","/",$class);//修改1,将_替换为/ ,如：System_UserAction=>System/UserAction
...
function A($name,$layer='',$common=false) {
    ...
    $class      =   basename($name.$layer);
    $nclass=str_replace("_","",strrchr($class,"_"));//修改2,去掉_之前的字符串，如：System_UserAction=>UserAction
    if(!empty($nclass))
    	$class=$nclass;
